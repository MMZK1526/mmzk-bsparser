...
many :: Alternative t => Foldable t => Monad m
     => BSParserT e m a -> BSParserT e m (t a) -> BSParserT e m (t a)
many p q = P.choice [liftA2 ((<|>) . pure) p (P.prune >> many p q), q]
{-# NOINLINE [2] many #-}
...
some p q = liftA2 ((<|>) . pure) p 
         $ P.choice [liftA2 ((<|>) . pure) p (P.prune >> many p q), q]
{-# NOINLINE [2] some #-}
...
manyS :: Alternative t => Foldable t => Monad m
      ...
      -> (BSParserT e m (t a) -> BSParserT e m (t a))
manyS pCPS q = P.choice [pCPS $ P.choice [manyS pCPS q, q], q]
{-# NOINLINE [2] manyS #-}
...
someS :: Alternative t => Foldable t => Monad m
      => (BSParserT e m (t a) -> BSParserT e m (t a))
      -> (BSParserT e m (t a) -> BSParserT e m (t a))
someS pCPS q = pCPS $ P.choice [manyS pCPS q, q]
{-# NOINLINE [2] someS #-}
...
manyL p q = P.choice [liftA2 (:) p (P.prune >> many p q), q]

someL :: Monad m => BSParserT e m a -> BSParserT e m [a] -> BSParserT e m [a]
someL p q = liftA2 (:) p $ P.choice [liftA2 (:) p (P.prune >> many p q), q]
